...
class DeltaXYOffsetCoder_Transformer(BaseBBoxCoder):
    # image_size：图像的尺寸，默认是 (200, 200)
    # target_means：编码时的均值，默认是 (0., 0.)
    # target_stds：编码时的标准差，默认是 (0.5, 0.5)
    def __init__(self,
                 image_size=(200, 200),
                 target_means=(0., 0.),
                 target_stds=(0.5, 0.5)):
    ...
    def encode(self, gt_offsets):
        if isinstance(gt_offsets, list):
            gt_offsets = gt_offsets[0]
        # 将真实偏移（gt_offsets）相对于参考框（bboxes）进行标准化编码，
        # 包括减去 mean，再除以 std，得到 encoded_offsets
        bboxes = torch.Tensor([[0, 0, self.image_size[0], self.image_size[1]]]).to(gt_offsets.device).repeat(
            gt_offsets.size(0), 1)
        encoded_offsets = offset2delta(bboxes, gt_offsets, self.means, self.stds)
        return encoded_offsets
    ...
    def decode(self,
        ...
        if isinstance(pred_offsets, list):
            pred_offsets = pred_offsets[0]
        bboxes = torch.Tensor([[0, 0, self.image_size[0], self.image_size[1]]]).to(pred_offsets.device).repeat(
            pred_offsets.size(0), 1)
        decoded_offsets = delta2offset(bboxes, pred_offsets, self.means, self.stds,
                                       max_shape, wh_ratio_clip)
...
class DeltaXYOffsetCoder_Transformer2(BaseBBoxCoder):
    ...
    def encode(self, gt_offsets):
        if isinstance(gt_offsets, list):
            gt_offsets = gt_offsets[0]
        bboxes = torch.Tensor([[self.image_size[0], self.image_size[1]]]).to(gt_offsets.device).repeat(
            gt_offsets.size(0), 1)
        # encoded_offsets = offset2delta(bboxes, gt_offsets, self.means, self.stds)
