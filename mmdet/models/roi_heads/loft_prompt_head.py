...
class LoftPromptHead(StandardRoIHead, OffsetTestMixin):
    ...
    def aug_test2(self, x, proposal_list, img_metas, rescale=False):
        # --------------------------------------------------------
        # 将 proposal_list（一般为 RPN 或 GT 的检测框）转换为 xywh 格式
        # 1. bbox2roi：将每张图像的 proposal 拼接成 RoI 张量 (N, 5) [batch_ind, x1, y1, x2, y2]
        # 2. roi2bbox：再还原为 [x1, y1, x2, y2] 形式
        # 3. xyxy2xywh：将坐标转换为 [x, y, w, h]
        # --------------------------------------------------------
        return_box = xyxy2xywh(roi2bbox(bbox2roi(proposal_list))[0])
        # --------------------------------------------------------
        # 若启用了噪声增强（self.noise_box 非空），
        # 则对 proposal_list 中的框加入随机扰动（如平移或缩放）
        # --------------------------------------------------------
        ...
        # --------------------------------------------------------
        # 如果启用了 inference_aug（即推理增强），
        # 会通过复制 proposal N 次进行多次预测并融合
        # --------------------------------------------------------
        if self.inference_aug:
            num = 100  # 重复次数
            # duplicate_tensor：复制 proposal_list[0] 中的 bbox num 次
            proposal_list2 = [duplicate_tensor(proposal_list[0], num)]
            # 将 bbox 转换为 RoI 格式（N×num, 5）
            ...
            # --------------------------------------------------------
            # offset_fusion：对多次预测的偏移结果进行融合
            # model='max' 表示使用逐像素取最大值融合策略
            # --------------------------------------------------------
            offset_results = self.offset_head.offset_fusion(offset_results['offset_pred'], model='max')
            # --------------------------------------------------------
            # post_fusion：对融合后的结果再进行一次平滑/均值融合
            # num+1 为样本数量
            # --------------------------------------------------------
        ...
        # get_out：对 mask 结果进行后处理（例如 resize、裁剪、拼接回原图尺寸等）
