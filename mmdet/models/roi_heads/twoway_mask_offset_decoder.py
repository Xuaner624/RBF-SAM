...
class MaskDecoder_seg(nn.Module):
    ...
    def __init__(
        ...
        self.offset_aug_query = nn.ModuleList(
            nn.Embedding(1, transformer_dim) for i in range(self.offset_aug_length)) if offset_aug else None
        ...
        if self.offset_aug_length:
            ...
            self.offset_aug_fc_offset = nn.ModuleList(
                nn.Linear(hidden_dim, 2, False) for i in range(self.offset_aug_length))
    ...
    def forward_test(
        ...
        masks, prob, offset, aug_offsets = self.predict_offset_masks(image_embeddings, image_pe,
                                                                     sparse_prompt_embeddings,
                                                                     dense_prompt_embeddings, )
    ...
    def forward_train(self,
        ...
        masks, prob, offset, offset_aug = self.predict_offset_masks(image_embeddings, image_pe,
    ...
    def predict_masks(
        ...
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings),
                           dim=1)  # (n, 5+2, 256) <- (n, 5, 256) + (n, 2, 256)
        ...
        hs, src = self.transformer(src, pos_src,
                                   tokens)  # queries:(1,7,256) ,  keys:(1,4096,256)  <-  (图片特征， 位置信息，（output，prompt）)
        ...
        masks = (hyper_in @ upscaled_embedding.view(b, c, h * w)).view(b, -1, h,
                                                                       w)  # [1,4,256,256] <- [1,4,256*256] <- [1,4,32] * [1,32,256*256]
