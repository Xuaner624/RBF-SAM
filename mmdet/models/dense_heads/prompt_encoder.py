...
@HEADS.register_module()
class PromptEncoder_Robust(nn.Module):
    def __init__(
            self,
            embed_dim: int,
            image_embedding_size: Tuple[int, int],
        ...
        # --------------- robust point embedding ---------------
        self.robust_box_types: int = 6
        robust_box_embeddings = [nn.Embedding(1, embed_dim) for i in range(self.robust_box_types)]
        self.robust_box_embeddings = nn.ModuleList(robust_box_embeddings)
    ...
    def _embed_boxes_roof(self, boxes: torch.Tensor):
        """Embeds box prompts."""
        boxes = boxes + 0.5  # Shift to center of pixel
        coords = boxes.reshape(-1, 2, 2)
        corner_embedding = self.pe_layer.forward_with_coords(coords, self.input_image_size)
        corner_embedding[:, 0, :] += self.robust_box_embeddings[0].weight
        corner_embedding[:, 1, :] += self.robust_box_embeddings[1].weight
        return corner_embedding

    def _embed_boxes_footprint(self, boxes: torch.Tensor):
        """Embeds box prompts."""
        boxes = boxes + 0.5  # Shift to center of pixel
        coords = boxes.reshape(-1, 2, 2)
        corner_embedding = self.pe_layer.forward_with_coords(coords, self.input_image_size)
        corner_embedding[:, 0, :] += self.robust_box_embeddings[2].weight
        corner_embedding[:, 1, :] += self.robust_box_embeddings[3].weight
        return corner_embedding

    def _embed_boxes_perturb(self, boxes: torch.Tensor):
        """Embeds box prompts."""
        boxes = boxes + 0.5  # Shift to center of pixel
        coords = boxes.reshape(-1, 2, 2)
        corner_embedding = self.pe_layer.forward_with_coords(coords, self.input_image_size)
        corner_embedding[:, 0, :] += self.robust_box_embeddings[4].weight
        corner_embedding[:, 1, :] += self.robust_box_embeddings[5].weight
        return corner_embedding
    ...
    def forward(
        ...
        if boxes is not None:
            if box_type == 0:
                box_embeddings = self._embed_boxes(boxes)
            elif box_type == 1:  # roof
                box_embeddings = self._embed_boxes_roof(boxes)
            elif box_type == 2:  # footprint
                box_embeddings = self._embed_boxes_footprint(boxes)
            elif box_type == 3:  # perturb
                box_embeddings = self._embed_boxes_perturb(boxes)
            sparse_embeddings = torch.cat([sparse_embeddings, box_embeddings], dim=1)
