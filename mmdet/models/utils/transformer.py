...
@HEADS.register_module()
class Deformable_TwoWayTransformer(nn.Module):
    def __init__(
            self,
            depth: int,
            embedding_dim: int,
        ...
        for i in range(depth):
            self.layers.append(
                Deformable_TwoWayAttentionBlock(
                    embedding_dim=embedding_dim,
                    num_heads=num_heads,
                    mlp_dim=mlp_dim,
                    activation=activation,
    ...
    def forward(
        ...
        for layer in self.layers:
            queries, keys = layer(
                ...
                query_pe=point_embedding,
                key_pe=image_pe,
                vit_sampled=vit_sample_feature,
            )
...
class Deformable_TwoWayAttentionBlock(nn.Module):
    def __init__(
            self,
            embedding_dim: int,
            num_heads: int,
        ...
        self.cross_attn_token_to_image = Deformable_Attention(
            embedding_dim, num_heads, downsample_rate=attention_downsample_rate
        )
        self.norm2 = nn.LayerNorm(embedding_dim)
    ...
    def forward(
            self, queries: Tensor, keys: Tensor, query_pe: Tensor, key_pe: Tensor, vit_sampled: Tensor
    ):  # -> Tuple[Tensor, Tensor]
        # Self attention block
        if self.skip_first_layer_pe:
            queries = self.self_attn(q=queries, k=queries, v=queries)
        ...
        q = queries + query_pe
        k = keys + key_pe
        attn_out = self.cross_attn_token_to_image(q=q, k=k, v=keys, vit_sampled=vit_sampled)
        queries = queries + attn_out
        queries = self.norm2(queries)
...
class Deformable_Attention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, vit_sampled: Tensor):  # -> Tensor
        alpha_v = self.mlp_v(q[:, 1:2, :])
        alpha_v = F.softmax(alpha_v, dim=-1)
        a1_v = alpha_v[..., 0].unsqueeze(-1)
        a2_v = alpha_v[..., 1].unsqueeze(-1)
        alpha_k = self.mlp_k(q[:, 1:2, :])
        alpha_k = F.softmax(alpha_k, dim=-1)
        a1_k = alpha_k[..., 0].unsqueeze(-1)
        a2_k = alpha_k[..., 1].unsqueeze(-1)
        v = a1_v * vit_sampled + a2_v * v
        k = a1_k * vit_sampled + a2_k * k

        # Input projections
        q = self.q_proj(q)  # 1,9,128
        k = self.k_proj(k)  # 1,4096,128
        v = self.v_proj(v)  # 1,4096,128
        # Separate into heads
        q = self._separate_heads(q, self.num_heads)  # 1,8,9,16
        k = self._separate_heads(k, self.num_heads)  # 1,8,4096,16
