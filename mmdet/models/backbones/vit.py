...
class ImageEncoderViT(nn.Module):
    ...
    #     for blk in self.blocks:
    #         x = blk(x)
    #         if blk.window_size == 0 and interm_embedding is None:
    #             # 第一次满足条件就保存
    #             interm_embedding = self.neck(x.permute(0, 3, 1, 2))
    #     # 通过了如上transformer blocks 依然为（1,64,64,768)
    #     x = self.neck(x.permute(0, 3, 1, 2))
    #     # （1,64,64,768) -> (1,256,64,64)
...
@BACKBONES.register_module()
class ImageEncoderViT_Robust(nn.Module):
    def __init__(
            self,
            img_size: int = 1024,
            patch_size: int = 16,
    ...
    def forward(self, x: torch.Tensor):  # -> torch.Tensor
        ...
        for blk in self.blocks:
            ...
            if blk.window_size == 0 and interm_embedding is None:
                # 第一次满足条件就保存
                interm_embedding = x.permute(0, 3, 1, 2)
        # 通过了如上transformer blocks 依然为（1,64,64,768)
        x = self.neck(x.permute(0, 3, 1, 2))
        # （1,64,64,768) -> (1,256,64,64)
        return x, interm_embedding
