...
def perturb_bbox(bboxes, max_shift=4, img_size=1024):
    ...
    # print("1 box perturbation:", shift[0])
    # print("2 box perturbation:", shift[1])
...
class obm_seg(BaseDetector):
    ...
    def forward_train(self,
        ...
        if prompt_len > self.train_cfg['max_num']:
            ...
            gt_offsets[0] = gt_offsets[0][loc, :]
        # ----------- 图像编码与 Prompt 编码 -----------
        features = self.image_encoder(img)  # 提取图像特征
        losses = dict()  # 保存loss
        sparse_embeddings, dense_embeddings = self.prompt_encoder(
            points=None,
            boxes=gt_bboxes[0],  # 用bbox作为prompt
            masks=None,
        )
        # ----------- 送入 mask_decoder 计算loss -----------
    ...
    def forward_test(self,
        ...
        # ----------- 通道合并（将前两通道平均为1个mask，后面通道平均为1个mask）-----------
        ...
        return return_box[0][0], masks, offset, return_box_robust[0][0]
    ...
    def forward_dummy(self, img):
        """Used for computing network FLOPs (模型复杂度计算用).
