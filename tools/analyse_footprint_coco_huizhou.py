...
ann = COCO('/root/autodl-tmp/data/BONAI/coco/building_seg_obm_huizhou_test.json')

def measure_detail(pred_path, anns=ann):
    preds = json.load(open(pred_path))
    range_len = []
    txt = os.path.join('/root/autodl-tmp/RBF-SAM-main/results/rbf_sam_huizhou/',
...
# python tools/analyse_footprint_coco_huizhou.py
if __name__ == '__main__':
    SCALE = 1.0
    pred_path = [
        '/root/autodl-tmp/RBF-SAM-main/results/rbf_sam_huizhou/rbf_sam_huizhou.json',
    ]
    COCO_path = [
    ...
    with warnings.catch_warnings():
        ...
        if not os.path.exists('results/rbf_sam_huizhou'):
            os.mkdir('results/rbf_sam_huizhou')
        print(pred_path)
