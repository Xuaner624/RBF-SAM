...
def decode_polygon(ann):
    rle = maskUtils.frPyObjects(ann, 512, 512)
    rle = maskUtils.merge(rle)
    binary_mask = maskUtils.decode(rle)
    return binary_mask
...
SAVE_PATH = '/root/autodl-tmp/RBF-SAM-main/results/rbf_sam_omnicity_vis'
ANN_COCO = COCO(
...
def _draw_boundary_offset(id, coco: COCO, save_dir=SAVE_PATH):
    ...
    segs = np.zeros((512, 512))

    buildings = []
    foot_segs = np.zeros((512, 512))
    image = cv2.imread(os.path.join('/root/autodl-tmp/data/OmniCityView3WithOffset/test/images', img['file_name']))
    ...
    for i, ann in enumerate(anns):
        ...
        pred_offset = np.array(ann['offset']) / 2
        """
        example：
        "roof_bbox": [
            10.0,
        ...
        seg_ = cv2.warpAffine(seg, translation_matrix, (512, 512))
...
if __name__ == '__main__':
    vis_boundary_offset('/root/autodl-tmp/RBF-SAM-main/results/rbf_sam_omnicity/rbf_sam_omnicity.json',
                        '/root/autodl-tmp/RBF-SAM-main/results/rbf_sam_omnicity_vis')
