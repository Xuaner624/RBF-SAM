...
def decode_mask(mask, size=1024):
...
ann = COCO('/root/autodl-tmp/data/BONAI/coco/bonai_shanghai_xian_test.json')

def measure_detail(pred_path, anns=ann):
    preds = json.load(open(pred_path))
    range_len = []
    txt = os.path.join('/root/autodl-tmp/RBF-SAM-main/results/rbf_sam_bonai/',
...
if __name__ == '__main__':
    SCALE = 1.0
    pred_path = [
        '/root/autodl-tmp/RBF-SAM-main/results/rbf_sam_bonai/rbf_sam_bonai.json',
    ]
    COCO_path = [
        '/root/autodl-tmp/data/BONAI/coco/bonai_shanghai_xian_test.json',
    ]
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        if not os.path.exists('results/rbf_sam_bonai'):
            os.mkdir('results/rbf_sam_bonai')
        print(pred_path)
